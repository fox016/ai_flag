...
const int MAX_OCCGRID_WIDTH = 100;
...
typedef struct occgrid_t
{
	...
	int xDim;
	int yDim;
	char grid[MAX_OCCGRID_WIDTH][MAX_OCCGRID_WIDTH];
} occgrid_t;
...
public:
	...
	// Get occgrid of tank at given index
	bool get_occgrid(int index, occgrid_t *occgrid)
	{
		char char_buff[20];
		sprintf(char_buff, " %d", index);	
		...
		occgrid->xPos = atof(v.at(1).substr(0, v.at(1).find_first_of(',')).c_str());
		occgrid->yPos = atof(v.at(1).substr(v.at(1).find_first_of(',') + 1).c_str());
//		cout << occgrid->xPos << endl;
//		cout << occgrid->yPos << endl;
		v.clear();
		v=ReadArr();
		if(v.at(0)!="size") {
			return false;
		}
		occgrid->xDim = atof(v.at(1).substr(0, v.at(1).find_first_of('x')).c_str());
		occgrid->yDim = atof(v.at(1).substr(v.at(1).find_first_of('x') + 1).c_str());
//		cout << occgrid->xDim << endl;
//		cout << occgrid->yDim << endl;
		int xGrid = 0;
		int yGrid = 0;
		for(xGrid = 0; xGrid < occgrid->xDim; xGrid++)
		{
			v.clear();
			v=ReadArr();	
			for(yGrid = 0; yGrid < occgrid->yDim; yGrid++)
			{
				occgrid->grid[xGrid][yGrid] = v.at(0)[yGrid];
//				cout << xGrid << ", " << yGrid << " = "
//					<< occgrid->grid[xGrid][yGrid] << endl;
			}
		}
		v.clear();
		v=ReadArr();	
