...
//	initWorld(&MyTeam);
	...
	else
	{
		...
		for(index = 0; index < TANK_COUNT; index = (index + 1) % TANK_COUNT)
		{
			...
			bool gotTanks = MyTeam.get_mytanks(&myTanks);
			while(!gotTanks)
			{
				gotTanks = MyTeam.get_mytanks(&myTanks);
			}
			...
			MyTeam.speed(index, controller.TurnTank(index, angle));
			MyTeam.shoot(index);
		}
	}
